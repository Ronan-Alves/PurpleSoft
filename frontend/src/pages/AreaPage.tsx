...
function SortableTaskTable({ tasks, selectedTaskId, onSelectTask, showAdmissionStage = false }: { tasks: ApiTask[]; selectedTaskId?: number | null; onSelectTask?: (task: ApiTask) => void; showAdmissionStage?: boolean }) {
  ...
  const deadlineClass = (deadline?: string | null) => !deadline ? "" : deadline < todayIso ? "overdue" : deadline === todayIso ? "due-today" : "future";
  const formatTableDate = (value?: string | null) => { if (!value) return "—"; const [year, month, day] = value.slice(0, 10).split("-"); return year && month && day ? `${day}/${month}/${year}` : value; };
  const filteredTasks = tasks.filter((task) => {
    const term = search.trim().toLocaleLowerCase();
    if (!term) return true;
    return [task.task_code ?? `T-${String(task.id).padStart(6, "0")}`, task.client_name, task.employee_name, stationLabel(task.station_id), currentStageLabel(task), task.priority, task.requested_at, task.deadline, formatTableDate(task.requested_at), formatTableDate(task.deadline)].some((value) => value?.toLocaleLowerCase().includes(term));
  });
...
function ManagerDashboardPage() {
  ...
  type Analytics = { periodDays: number; summary: { active: number; overdue: number; completed: number; slaRate: number; activeOnTimeRate: number; averageCycleDays: number; efficiencyChange: number; checklistPending: number; unassigned: number; dueSoon: number; netFlow: number }; trend: { label: string; created: number; completed: number }[]; employees: { name: string; active: number; completed: number; overdue: number; slaRate: number; averageCycleDays: number; efficiencyChange: number }[]; stations: { stationId: string; label: string; active: number; completed: number; overdue: number }[]; aging: Distribution[]; priorities: Distribution[]; stages: Distribution[]; monthlyDistribution: { month: string; monthLabel: string; name: string; admissions: number; terminations: number; vacations: number; payroll: number; total: number }[] };
  ...
  const maxTrend = Math.max(...(analytics?.trend.flatMap((item) => [item.created, item.completed]) ?? [1]), 1);
  ...
  const receivedInPeriod = analytics?.trend.reduce((total, item) => total + item.created, 0) ?? 0;
  const distributionMonths = Array.from(new Map((analytics?.monthlyDistribution ?? []).map((item) => [item.month, item.monthLabel])).entries());
  const activeDistributionMonth = distributionMonth || distributionMonths[0]?.[0] || "";
  const monthlyRows = analytics?.monthlyDistribution.filter((item) => item.month === activeDistributionMonth) ?? [];
  ...
  return (
    <AreaFrame title="Gestão do Setor" subtitle="Produtividade, cumprimento de prazos e evolução da eficiência do Departamento Pessoal." icon={<BarChart3 />} smallHeader backTo="/area/pessoal" backLabel="Sair">
      ...
      {analytics && <div className="management-dashboard">
        ...
        <section className="management-health">
          <article><span>Checklist pendente {help("Demandas que ainda aguardam documentos, formulários ou outros insumos para liberação.")}</span><strong>{analytics.summary.checklistPending}</strong></article>
          <article><span>Sem responsável {help("Demandas ativas que ainda não foram atribuídas a um colaborador.")}</span><strong>{analytics.summary.unassigned}</strong></article>
          <article><span>Prazo próximo {help("Demandas que vencem hoje ou dentro da janela de alerta configurada pelo gestor.")}</span><strong>{analytics.summary.dueSoon}</strong></article>
          <article className={analytics.summary.netFlow > 0 ? "danger" : "positive"}><span>Variação da fila {help("Diferença entre as demandas recebidas e concluídas no período. Positivo significa que a fila cresceu; negativo significa que diminuiu.")}</span><strong>{analytics.summary.netFlow > 0 ? "+" : ""}{analytics.summary.netFlow}</strong><small>{receivedInPeriod} recebidas · {analytics.summary.completed} concluídas</small></article>
          <article><span>Concluídas no prazo {help("Percentual das demandas finalizadas no período cuja conclusão ocorreu até a data limite.")}</span><strong>{analytics.summary.slaRate.toLocaleString("pt-BR")}%</strong><small>das {analytics.summary.completed} concluídas</small></article>
        </section>
        <section className="management-dashboard-grid">
          <section className="ops-panel management-trend"><div><h3>Entrada versus saída</h3><small>Demandas recebidas e concluídas em cada intervalo semanal.</small></div><div className="management-chart-legend"><span><i className="created" /> Recebidas</span><span><i className="completed" /> Concluídas</span></div><div className="management-bars">{analytics.trend.map((item) => <article key={item.label}><strong>{item.created}/{item.completed}</strong><div><span className="created" style={{ height: `${Math.max(item.created / maxTrend * 100, item.created ? 8 : 2)}%` }} /><span className="completed" style={{ height: `${Math.max(item.completed / maxTrend * 100, item.completed ? 8 : 2)}%` }} /></div><small>{item.label}</small></article>)}</div></section>
        ...
        <section className="management-distributions">
          <section className="ops-panel"><div><h3>Tempo em aberto</h3><small>Tempo transcorrido desde a solicitação das demandas ainda não concluídas.</small></div>{analytics.aging.map((item) => <article key={item.label}><span>{item.label}</span><div><i style={{ width: `${analytics.summary.active ? item.count / analytics.summary.active * 100 : 0}%` }} /></div><strong>{item.count}</strong></article>)}</section>
          <section className="ops-panel"><div><h3>Pressão por prioridade</h3><small>Distribuição das demandas ativas por prioridade.</small></div>{analytics.priorities.map((item) => <article key={item.label}><span>{item.label}</span><div><i style={{ width: `${analytics.summary.active ? item.count / analytics.summary.active * 100 : 0}%` }} /></div><strong>{item.count}</strong></article>)}</section>
          <section className="ops-panel"><div><h3>Gargalo das admissões</h3><small>Etapa atual das admissões ainda abertas.</small></div>{analytics.stages.map((item) => <article key={item.label}><span>{item.label}</span><div><i style={{ width: `${Math.max(...analytics.stages.map((stage) => stage.count), 1) ? item.count / Math.max(...analytics.stages.map((stage) => stage.count), 1) * 100 : 0}%` }} /></div><strong>{item.count}</strong></article>)}</section>
        </section>
        <section className="ops-panel management-monthly-distribution"><div className="customer-list-header"><div><h3>Distribuição mensal por colaborador</h3><small>Tarefas solicitadas no mês e atribuídas a cada colaborador, separadas por tipo.</small></div><label className="inline-filter">Mês<select value={activeDistributionMonth} onChange={(event) => setDistributionMonth(event.target.value)}>{distributionMonths.map(([month, label]) => <option value={month} key={month}>{label}</option>)}</select></label></div><div className="company-table-wrap"><table className="company-table"><thead><tr><th>Colaborador</th><th>Admissões</th><th>Rescisões</th><th>Férias</th><th>Folhas</th><th>Total no mês</th></tr></thead><tbody>{monthlyRows.map((row) => <tr key={`${row.month}-${row.name}`}><td><strong>{row.name}</strong></td><td>{row.admissions}</td><td>{row.terminations}</td><td>{row.vacations}</td><td>{row.payroll}</td><td><strong>{row.total}</strong></td></tr>)}{monthlyRows.length === 0 && <tr><td colSpan={6} className="empty-state">Não há tarefas atribuídas neste mês.</td></tr>}</tbody></table></div></section>
        <section className="ops-panel management-employees"><div className="customer-list-header"><div><h3>Produtividade por colaborador</h3><small>Resultados individuais no período e comparação com o período anterior.</small></div></div><div className="company-table-wrap"><table className="company-table"><thead><tr><th>Colaborador</th><th>Ativas</th><th>Concluídas</th><th>Atrasadas</th><th>No prazo</th><th>Tempo médio</th><th title="Variação na quantidade concluída em relação ao período anterior de mesma duração.">Variação de entregas</th></tr></thead><tbody>{analytics.employees.map((employee) => <tr key={employee.name}><td><strong>{employee.name}</strong></td><td>{employee.active}</td><td>{employee.completed}</td><td>{employee.overdue}</td><td>{employee.slaRate.toLocaleString("pt-BR")}%</td><td>{employee.averageCycleDays.toLocaleString("pt-BR")} dias</td><td><span className={`efficiency-badge ${employee.efficiencyChange < 0 ? "negative" : "positive"}`}>{efficiencyLabel(employee.efficiencyChange)}</span></td></tr>)}{analytics.employees.length === 0 && <tr><td colSpan={7} className="empty-state">Ainda não há tarefas atribuídas suficientes para análise individual.</td></tr>}</tbody></table></div></section>
      </div>}
    </AreaFrame>
  );
}
